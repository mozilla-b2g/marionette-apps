...
var _Mgmt = require(__dirname + '/_mgmt'),
    Mgmt = require(__dirname + '/mgmt'),
    fs = require('fs');
...
function Apps(client) {
  ...
  this._mgmt = new _Mgmt(this, client);
}
module.exports = Apps;
...
Apps.setup = function(client, options, cb) {
  ...
  apps._mgmt.prepareClient(function(err) {
    if (err) {
      throw err;
    }
...
Apps.prototype = {
  ...
  /**
   * A magical extension to the apps mgmt api with some nice abstractions
   * related to app management.
